            ...
            <div class="d-flex justify-content-between align-items-center breaking-news bg-white">
                ...
                <marquee class="news-scroll" behavior="scroll" loop="100" scrolldelay="1"
                    ...
                    @foreach ($breaking_news as $item)
                        <a href="{{ route('news.detail', [app()->getLocale(), $item->slug])}}">{{ $item->title }}</a>
                        <span class="dot"></span>
                    @endforeach
...
{{--
                    <a onClick="window.open('http://www.facebook.com/sharer.php?s=100&amp;p[title]={{ route('news.detail', [app()->getLocale(), $item->slug])}}&amp;p[url]={{ route('news.detail', [app()->getLocale(), $item->slug])}}&amp;&p[images][0]={{ asset('assets/postImages')."/".$news->top_image }}', 'sharer', 'toolbar=0,status=0,width=548,height=325');" target="_parent" href="javascript: void(0)">
                        Share our Facebook page!
                    </a> --}}

                        <iframe src="https://www.facebook.com/plugins/share_button.php?href={{ route('news.detail', [app()->getLocale(), $news->slug])}}"
